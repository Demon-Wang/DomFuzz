...
namespace {
	...
	static void dict2file(u8 *mem, u32 len) {
	...
	bool ModuleSanitizerCoverage::instrumentModule(
		...
		if (getenv("AFL_DIRECT")) {
			...
			if(getenv("DOM_FILE")){
				std::ifstream domsfile(getenv("DOM_FILE"));
				std::string line;
			...
			if(getenv("DISTANCE_FILE")){
				std::ifstream cf(getenv("DISTANCE_FILE"));
				if (cf.is_open()) {
					std::string line;
					...
					FATAL("Unable to find %s.", getenv("DISTANCE_FILE"));
					return false;
				}
			}
			if(getenv("TARGET_FILE")){
				std::ifstream targetsfile(getenv("TARGET_FILE"));
				std::string line;
...
#ifdef __x86_64__
			IntegerType *LargestType = Int64Tyi;
			ConstantInt *MapCntLoc = ConstantInt::get(LargestType, MAP_SIZE + 8);
			ConstantInt *MapDomCountLoc = ConstantInt::get(Int32Tyi, MAP_SIZE + 16);
			ConstantInt *MapDestCountLoc = ConstantInt::get(Int32Tyi, MAP_SIZE + 20);
#else
			IntegerType *LargestType = Int32Tyi;
			ConstantInt *MapCntLoc = ConstantInt::get(LargestType, MAP_SIZE + 4);
			ConstantInt *MapDomCountLoc = ConstantInt::get(Int32Tyi, MAP_SIZE + 8);
			ConstantInt *MapDestCountLoc = ConstantInt::get(Int32Tyi, MAP_SIZE + 12);
#endif
			ConstantInt *MapDistLoc = ConstantInt::get(LargestType, MAP_SIZE);
			ConstantInt *One = ConstantInt::get(LargestType, 1);
			ConstantInt *One32 = ConstantInt::get(Int32Tyi, 1);
			...
			for (auto &F : M) {
				...
				for (auto &BB : F) {
					...
					if((distance!=-1)||is_target||is_dom){
						...
						if (is_target){
							...
							MapDestCount->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(Ctx, None));
						...
						if (is_dom){
							...
							MapDomCount->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(Ctx, None));
			...
			OKF("Instrumented %u locations %u doms, %u des.",inst_blocks,dom_count, des_count);
		}
		...
		if (getenv("DICT2FILE")) {
			...
			ptr = getenv("DICT2FILE");

			if (!ptr || *ptr != '/')
				FATAL("DICT2FILE is not set to an absolute path: %s", ptr);
